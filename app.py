...
# 4. Inicialização do Cliente da API (Suporta Local com secrets.toml e Produção)
api_key = None

if "GEMINI_API_KEY" in st.secrets:
    api_key = st.secrets["GEMINI_API_KEY"]
else:
    # Backup seguro caso o arquivo .streamlit/secrets.toml local não seja lido
    api_key = st.environ.get("GEMINI_API_KEY", None)
...
try:
    # A nova biblioteca gerencia o roteamento correto automaticamente passando apenas a key
    client = genai.Client(api_key=api_key)
except Exception as e:
    st.error(f"❌ Erro ao inicializar o cliente Gemini: {e}")
    st.stop()

# 5. Interfaz de Usuário (UI) - Parâmetros inválidos limpos
...
if st.button("Gerar anúncio", type="primary", use_container_width=True):
    if not product:
        ...
        with st.spinner("Gerando conteúdo estratégico..."):
            ...
            try:
                # Mudança estratégica para o modelo de produção estável 'gemini-2.5-flash'
                ...
                # Divisão limpa de blocos na interface
                ...
                # Montagem do arquivo para download
                ...
                if e.code == 503:
                    st.error("⚠️ Os servidores do Gemini estão sob alta demanda temporária. Por favor, aguarde alguns segundos e clique em 'Gerar anúncio' novamente.")
                elif e.code == 404:
                    st.error("⚠️ Erro 404: O modelo especificado não foi encontrado. Certifique-se de usar 'gemini-2.5-flash'.")
                elif e.code == 401:
